    ...
    unsigned int out_streams)
{
    ...
    else if (implementation == "GPS_L1_CA_Double_Chip_Rate_Acquisition")
        {
            // Notice the class name here has NO underscores, matching your .h file
            return std::make_unique<GpsL1CaDoubleChipRateAcquisition>(
                configuration,
                role,
                in_streams,
                 out_streams);
...
std::unique_ptr<TrackingInterface> get_trk_block(
    ...
    unsigned int out_streams)
{
    if (implementation == "GPS_L1_CA_Double_Chip_Rate_Tracking")
    {
        return std::make_unique<GpsL1CaDoubleChipRateTracking>(
            configuration,
            role,
            in_streams,
            out_streams);

...
pcps_acquisition::AcquisitionResult pcps_acquisition::first_vs_second_peak_statistic(uint32_t num_doppler_bins, int32_t doppler_max, int32_t doppler_step)
{
    ...
    // FIX FOR LEO PNT DOUBLE CHIP RATE:
    // The signal repeats twice per ms, creating a mathematically identical
    // twin peak at half the FFT size (4092 samples away).
    // We must also exclude this twin peak to get a meaningful secondPeak.
    int32_t twin_peak_idx = (result.index_time + d_effective_fft_size / 2) % d_effective_fft_size;
    int32_t excludeRangeIndex3 = twin_peak_idx - d_samplesPerChip;
    int32_t excludeRangeIndex4 = twin_peak_idx + d_samplesPerChip;
    
    if (excludeRangeIndex3 < 0)
        {
            excludeRangeIndex3 = d_effective_fft_size + excludeRangeIndex3;
        }
    else if (excludeRangeIndex4 >= static_cast<int32_t>(d_effective_fft_size))
        {
            excludeRangeIndex4 = excludeRangeIndex4 - d_effective_fft_size;
        }
        
    idx = excludeRangeIndex3;
    do
        {
            d_tmp_buffer[idx] = 0.0;
            idx++;
